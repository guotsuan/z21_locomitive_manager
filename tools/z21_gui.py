...
class Z21GUI:
    ...
    def __init__(self, root, z21_file: Path):
        ...
        self.current_loco_index: Optional[int] = None  # Index in z21_data.locomotives
        self.original_loco_address: Optional[int] = None  # Store original address for database lookup
    ...
    def setup_ui(self):
        ...
        ttk.Label(list_frame, text="Locomotives:", font=('Arial', 10, 'bold')).pack(anchor=tk.W)
        ...
        self.loco_listbox = tk.Listbox(listbox_frame, yscrollcommand=scrollbar.set, font=('Arial', 10))
        ...
        # Status label
        self.status_label = ttk.Label(left_frame, text="Loading...", relief=tk.SUNKEN)
    ...
    def setup_overview_tab(self):
        ...
        details_frame = ttk.LabelFrame(self.overview_frame, text="Locomotive Details", padding=10)
        details_frame.pack(fill=tk.X, padx=5, pady=5)
        
        # Name field
        ttk.Label(details_frame, text="Name:", width=15, anchor='e').grid(row=0, column=0, padx=5, pady=5, sticky='e')
        self.name_var = tk.StringVar()
        self.name_entry = ttk.Entry(details_frame, textvariable=self.name_var, width=40)
        self.name_entry.grid(row=0, column=1, padx=5, pady=5, sticky='ew')
        
        # Address field
        ttk.Label(details_frame, text="Address:", width=15, anchor='e').grid(row=1, column=0, padx=5, pady=5, sticky='e')
        self.address_var = tk.StringVar()
        self.address_entry = ttk.Entry(details_frame, textvariable=self.address_var, width=40)
        self.address_entry.grid(row=1, column=1, padx=5, pady=5, sticky='ew')
        
        # Max Speed field
        ttk.Label(details_frame, text="Max Speed:", width=15, anchor='e').grid(row=2, column=0, padx=5, pady=5, sticky='e')
        self.speed_var = tk.StringVar()
        self.speed_entry = ttk.Entry(details_frame, textvariable=self.speed_var, width=40)
        self.speed_entry.grid(row=2, column=1, padx=5, pady=5, sticky='ew')
        
        # Direction field (dropdown)
        ttk.Label(details_frame, text="Direction:", width=15, anchor='e').grid(row=3, column=0, padx=5, pady=5, sticky='e')
        self.direction_var = tk.StringVar()
        self.direction_combo = ttk.Combobox(details_frame, textvariable=self.direction_var, 
                                            values=['Forward', 'Reverse'], state='readonly', width=37)
        ...
        self.save_button = ttk.Button(button_frame, text="Save Changes", command=self.save_locomotive_changes)
    ...
    def setup_functions_tab(self):
        ...
        scrollbar = ttk.Scrollbar(self.functions_frame, orient="vertical", command=canvas.yview)
        ...
        root = self.root
        root.bind_all("<MouseWheel>", lambda e: on_mousewheel(e) if self.notebook.index(self.notebook.select()) == 1 else None, add='+')
        root.bind_all("<Button-4>", lambda e: on_mousewheel(e) if self.notebook.index(self.notebook.select()) == 1 else None, add='+')
        root.bind_all("<Button-5>", lambda e: on_mousewheel(e) if self.notebook.index(self.notebook.select()) == 1 else None, add='+')
...
"""
    ...
    def save_locomotive_changes(self):
        ...
        try:
            ...
            try:
                ...
                messagebox.showinfo("Success", "Locomotive details saved successfully to file!")
    ...
    def update_functions(self):
        ...
        cols = max(1, (canvas_width - 40) // card_width)  # Account for scrollbar and padding
        ...
        header_label.grid(row=0, column=0, columnspan=cols, sticky='ew', padx=5, pady=(10, 5))
    ...
    def save_function_changes(self):
        ...
        try:
            ...
            # Write changes back to file
            self.parser.write(self.z21_data, self.z21_file)
            messagebox.showinfo("Success", "All function changes saved successfully to file!")
        except Exception as write_error:
            messagebox.showerror("Write Error", 
                f"Failed to write changes to file: {write_error}\n\n"
                f"Changes have been saved in memory but not written to disk.")
    ...
    def get_available_icons(self):
        ...
        common_icons = ['light', 'bell', 'horn_two_sound', 'steam', 'whistle_long', 
    ...
    def add_new_function(self):
        ...
        func_num_var = tk.StringVar(value=str(self.get_next_unused_function_number()))
        ...
        icon_preview_label = ttk.Label(preview_frame, background='white', relief=tk.SUNKEN, borderwidth=2)
        ...
        # Icon selection
        ttk.Label(left_col, text="Icon:", width=12, anchor='e').grid(row=row, column=0, padx=3, pady=4, sticky='e')
        icon_combo = ttk.Combobox(left_col, textvariable=icon_var, width=20, state='readonly')
        ...
        # Function number
        ttk.Label(left_col, text="Function #:", width=12, anchor='e').grid(row=row, column=0, padx=3, pady=4, sticky='e')
        func_num_entry = ttk.Entry(left_col, textvariable=func_num_var, width=20)
        ...
        # Shortcut
        ttk.Label(right_col, text="Shortcut:", width=12, anchor='e').grid(row=row_right, column=0, padx=3, pady=4, sticky='e')
        shortcut_entry = ttk.Entry(right_col, textvariable=shortcut_var, width=20)
        shortcut_entry.grid(row=row_right, column=1, padx=3, pady=4, sticky='ew')
        row_right += 1
        
        # Button type
        ttk.Label(right_col, text="Button Type:", width=12, anchor='e').grid(row=row_right, column=0, padx=3, pady=4, sticky='e')
        button_type_combo = ttk.Combobox(right_col, textvariable=button_type_var, 
                                        values=['switch', 'push-button', 'time button'],
                                        state='readonly', width=17)
        button_type_combo.current(0)
        button_type_combo.grid(row=row_right, column=1, padx=3, pady=4, sticky='ew')
        row_right += 1
        
        # Time duration (only show for time button) - in right column
        time_label = ttk.Label(right_col, text="Time (s):", width=12, anchor='e')
        ...
        def update_time_visibility(*args):
            ...
            if button_type_var.get() == 'time button':
                time_label.grid(row=row_right, column=0, padx=3, pady=4, sticky='e')
                time_entry.grid(row=row_right, column=1, padx=3, pady=4, sticky='ew')
            else:
            ...
            try:
                ...
                if button_type == 2:  # time button
                    try:
                        ...
                        messagebox.showerror("Error", "Time duration must be a valid number.")
                        return
                else:
                    time_str = "0"
                ...
                if self.current_loco.function_details:
                    max_position = max(f.position for f in self.current_loco.function_details.values())
                ...
                messagebox.showinfo("Success", f"Function F{func_num} added successfully!")
        ...
        ttk.Button(button_frame, text="Add Function", command=save_function).pack(side=tk.RIGHT, padx=5)
    ...
    def edit_function(self, func_num: int, func_info: FunctionInfo):
        ...
        button_type_var = tk.StringVar(value=button_type_map.get(func_info.button_type, "switch"))
        time_var = tk.StringVar(value=func_info.time if func_info.time != "0" else "1.0")
        ...
        button_type_combo = ttk.Combobox(right_col, textvariable=button_type_var, 
                                        values=['switch', 'push-button', 'time button'],
                                        state='readonly', width=17)
        button_type_combo.current(['switch', 'push-button', 'time button'].index(button_type_var.get()))
        button_type_combo.grid(row=row_right, column=1, padx=3, pady=4, sticky='ew')
        row_right += 1
        ...
        def save_changes():
            ...
            try:
                ...
                if new_func_num != func_num:
                    ...
                    self.current_loco.function_details[new_func_num] = func_info
                ...
                messagebox.showinfo("Success", f"Function F{new_func_num} updated successfully!")
                
            except ValueError as e:
                messagebox.showerror("Error", f"Invalid input: {e}")
            except Exception as e:
                messagebox.showerror("Error", f"Failed to update function: {e}")
    ...
    def load_icon_image(self, icon_name: str = None, size: tuple = (80, 80)):
        """Load icon image with blue foreground and white background."""
        project_root = Path(__file__).parent.parent
        icons_dir = project_root / "icons"
        
        def convert_to_blue(img):
            """Convert icon to blue color on white background."""
            if not HAS_PIL:
                return img
            ...
            # Create blue version based on grayscale intensity
            blue_img = Image.new('RGBA', img.size)
            ...
            for x in range(img.size[0]):
                for y in range(img.size[1]):
                    ...
                    # Invert intensity: dark areas become bright blue, light areas become light blue
                    blue_intensity = 255 - intensity
                    # Preserve alpha channel
                    alpha = original_pixels[x, y][3] if img.mode == 'RGBA' else 255
                    # Create blue color (more blue in darker areas)
                    blue_pixels[x, y] = (
                        blue_intensity // 4,      # Red component (minimal)
                        blue_intensity // 2,      # Green component
                        blue_intensity,           # Blue component (full)
                        alpha                      # Alpha channel
                    )
                ...
                if icon_path.exists():
                    try:
                        if HAS_PIL:
                            ...
                            img = convert_to_blue(img)
                            
                            # Create white background
                            white_bg = Image.new('RGB', size, color='white')
                            
                            # Resize icon
                            icon_resized = img.resize(size, Image.Resampling.LANCZOS)
        ...
        # Use default icon (neutrals_normal.png) with blue color
        ...
        # Fallback: create a white square with blue border
        if HAS_PIL:
    ...
    def create_function_card(self, func_num: int, func_info):
        ...
        icon_frame.grid(row=0, column=0, padx=CARD_PADDING, pady=(CARD_PADDING, 2), sticky='')
        icon_frame.pack_propagate(False)
        
        # Load and display icon with blue color on white background
        ...
        if icon_image:
            ...
            # Fallback: white square with blue border
            fallback = tk.Label(
                icon_frame, 
                bg='white', 
                width=ICON_SIZE//8, 
        ...
        type_time_frame.grid(row=3, column=0, pady=(0, CARD_PADDING), sticky='')
        
        # Button type
        button_type_label = tk.Label(
            type_time_frame,
...
def main():
    ...
    parser.add_argument('file', type=Path, nargs='?', default=Path('z21_new.z21'),
