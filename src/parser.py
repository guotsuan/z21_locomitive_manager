...
class Z21Parser:
    ...
    def _parse_sqlite(self, zipfile_obj: zipfile.ZipFile, sqlite_path: str,
                      z21_file: Z21File):
        ...
        try:
            ...
            base_fields = [
                ...
                'traction_direction', 'image_name', 'drivers_cab',
                'description', 'full_name', 'railway', 'article_number',
                'decoder_type', 'build_year', 'buffer_lenght',
                'model_buffer_lenght', 'service_weight', 'model_weight',
                'rmin', 'ip', 'speed_display', 'type'
            ]

            # Add in_stock_since if it exists (try different possible column names)
            in_stock_since_field = None
            for field_name in [
                    'in_stock_since', 'inStockSince', 'in_stock_since_date'
            ]:
            ...
            for vehicle in vehicles:
                ...
                loco.buffer_length = vehicle[
                    'buffer_lenght'] or ''  # Note: typo in DB column name
                loco.model_buffer_length = vehicle[
                    'model_buffer_lenght'] or ''  # Note: typo in DB column name
                ...
                loco.active = bool(
                    vehicle['active'] if vehicle['active'] is not None else 1)
                loco.speed_display = vehicle['speed_display'] if vehicle[
                    'speed_display'] is not None else 0
                loco.rail_vehicle_type = vehicle['type'] if vehicle[
                    'type'] is not None else 0

                # Extract in_stock_since if field exists
                if in_stock_since_field:
                    try:
                        loco.in_stock_since = vehicle[
                            in_stock_since_field] or ''
            ...
            for func_elem in functions_elem.findall('function_element'):
                ...
                if func_num_elem is not None and func_num_elem.text:
                    ...
                    func_info = FunctionInfo(
                        ...
                        shortcut=shortcut_elem.text
                        if shortcut_elem is not None and shortcut_elem.text
                        else '',
                        position=int(position_elem.text)
                        if position_elem is not None and position_elem.text
                        else 0,
                        time=time_elem.text
                        if time_elem is not None and time_elem.text else '0',
    ...
    def write(self,
        ...
        with zipfile.ZipFile(self.file_path, 'r') as input_zip:
            ...
            if sqlite_file:
                ...
                written_path = self._write_sqlite(input_zip, sqlite_file,
                                                  z21_file, temp_output_path)
                ...
                raise NotImplementedError(
                    "Writing XML format not yet implemented")
        ...
        try:
            ...
            for loco in z21_file.locomotives:
                ...
                if hasattr(loco, '_vehicle_id') and loco._vehicle_id:
                    ...
                    cursor.execute(
                        "SELECT id FROM vehicles WHERE id = ? AND type = 0",
                        (vehicle_id, ))
                ...
                if not vehicle_id and not getattr(loco, '_is_new_import',
                                                  False):
                    cursor.execute(
                        """
                ...
                if vehicle_id:
                    ...
                    base_update_fields = [
                        'name = ?', 'address = ?', 'max_speed = ?',
                        'traction_direction = ?', 'full_name = ?',
                        'railway = ?', 'article_number = ?',
                        'decoder_type = ?', 'build_year = ?',
                        'model_buffer_lenght = ?', 'service_weight = ?',
                        'model_weight = ?', 'rmin = ?', 'ip = ?',
                        'drivers_cab = ?', 'description = ?', 'active = ?',
                        'speed_display = ?', 'type = ?', 'image_name = ?'
                    ]
                    base_values = [
                        loco.name,
                        loco.address,
                        ...
                        loco.model_buffer_length
                        or None,  # Note: typo in DB column name
                    ...
                    if in_stock_since_field:
                        base_update_fields.append(
                            f'{in_stock_since_field} = ?')
                        base_values.append(
                            getattr(loco, 'in_stock_since', '') or None)
                    ...
                    for func_num, func_info in loco.function_details.items():
                        ...
                        time_value = float(
                            func_info.time
                        ) if func_info.time and func_info.time != "0" else None
                        ...
                        if cursor.fetchone():
                            ...
                            cursor.execute(
                                ...
                                WHERE vehicle_id = ? AND function = ?
                            """,
                                (func_info.position, func_info.shortcut
                                 or '', time_value, func_info.image_name or '',
                                 func_info.button_type, vehicle_id, func_num))
                        else:
                            # Insert new function
                            cursor.execute(
                                """
                                ...
                                VALUES (?, ?, ?, ?, ?, ?, ?, 1, 1)
                            """, (vehicle_id, func_num, func_info.position,
                                  func_info.shortcut
                                  or '', time_value, func_info.image_name
                                  or '', func_info.button_type))
                else:
                    ...
                    """, (loco.rail_vehicle_type or 0, loco.name, loco.address,
                          ...
                          or None, loco.railway or None, loco.article_number
                          or None, loco.decoder_type or None, loco.build_year
                          or None, loco.buffer_length or None,
                          loco.model_buffer_length or None, loco.service_weight
                          or None, loco.model_weight or None, loco.rmin
                          or None, loco.ip or None, loco.speed_display or 0))
            ...
            for vehicle_id in vehicles_to_delete:
                ...
                cursor.execute("DELETE FROM functions WHERE vehicle_id = ?",
                               (vehicle_id, ))
                ...
                cursor.execute("DELETE FROM vehicles WHERE id = ?",
                               (vehicle_id, ))
            ...
            with zipfile.ZipFile(output_path, 'w',
                ...
                for item in input_zip.infolist():
                    if item.filename == sqlite_path:
                        ...
                        zip_info = zipfile.ZipInfo(filename=item.filename,
                        ...
                        is_image = item.filename.lower().endswith(
                        ...
                        if is_image:
                            ...
                            for used_img in used_image_files:
                                if (filename_only == used_img
                                        or item.filename == used_img
                                        or item.filename.endswith(used_img)
                                        or used_img in item.filename):
                                    is_used = True
                                    break
